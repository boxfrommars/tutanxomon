...
class Geo extends Core {
	...
	/**
		Return array of countries indexed by country code
			@return array
			@public
	**/
	...
	**/
	public static function weather($latitude,$longitude) {
		...
		if (is_null($result['weatherObservation'])) {
	...
	/**
		Return geolocation information, or FALSE on failure
			@return mixed
			@param $ip string
			@public
	**/
	public static function location($ip=NULL) {
		$result=json_decode(
			F3::http(
				'GET http://ipinfodb.com/ip_query.php?'.
				http_build_query(
					array(
						'output'=>'json',
						'ip'=>$ip?:NULL,
